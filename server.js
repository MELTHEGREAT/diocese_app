...
app.use(session({
    ...
    store: new MemoryStore({
        checkPeriod: 86400000 // Optional: Check period for expired sessions (in milliseconds)
    }),
    cookie: {
        // Cookie settings
    }
...
const logger = winston.createLogger({
  ...
  app.get('/toNotifications',ensureAuthenticated, (req, res) => {
...
app.use(session({
    secret: 'b9e868e5d118c6d8f0f2e507a6b7f27b9f18b76038c7e2a021c0a36eac20d831d350f8f584431ee0840a9f9d548ec5002d7bb23ecffdb1e38316231d832b26f5',
    resave: false,
    saveUninitialized: true,
    cookie: {
      path: '/',
                                             ...
                                             connection.query(countStaRitaUsersQuery, (staRitaUsersErr, staRitaUsersCountResult) => {
                                                 ...
                                                 if (user.role === 'admin') {
                                                   ...
                                                   console.log(req.session.user = {
                                                        id: user.id, // Set user ID if available
                                                        email: user.email // Set user email or any other relevant user data
                                                    });
                                                 ...
                                                 } else if (user.role === 'diocese_admin') {
                                                     req.session.isAuthenticated = true;
                                                    ...
                                                    fetchAndRenderFinancialReportsDioceseAdmin(res, totalUsers, adminList, userEmail);
                                                 } else if (user.role === 'superadmin') {
                                                    req.session.isAuthenticated = true;
                                                   ...
                                                   fetchAndRenderFinancialReportsSystemAdmin(res, totalUsers, adminList, userEmail);
                                                }
...
app.post('/add_donation', (req, res) => {
    const { amount, name, email, donation_date, confirmation_status, designation, additional_comments } = req.body;
  
    const sql = `INSERT INTO donations (amount, name, email, donation_date, confirmation_status, designation, additional_comments) VALUES (?, ?, ?, ?, ?, ?, ?)`;
    const values = [amount, name, email, donation_date, confirmation_status, designation, additional_comments];
    ...
    //   res.redirect('/toGiving'); // Redirect to home page or any other page
    });
  });
...
app.get('/toGiving',ensureAuthenticated, (req, res) => {
...
app.get('/toMembers', (req, res) => {
    ...
    const countCustomerServices = 'SELECT COUNT(*) AS contactMessagesCount FROM contact_message';
    const countScheduleRequestsQuery = 'SELECT COUNT(*) AS scheduleRequestCount FROM schedule_requests';
    connection.query('SELECT * FROM users WHERE role NOT IN (?, ?, ?) AND church = ?', ['superadmin', 'admin', 'diocese_admin', 'San Roque Parish Church'], (error, results) => {
...
app.post('/updateSchedule/:id', (req, res) => {
    ...
    // Fetch the user ID based on the userEmail
    const getUserIdQuery = 'SELECT id FROM users WHERE email = ?';
    connection.query(getUserIdQuery, [userEmail], (userErr, userResults) => {
        if (userErr || userResults.length === 0) {
            console.error('Error fetching user ID:', userErr || 'User not found');
            res.status(500).send('Internal Server Error');
        } else {
                ...
                if (updateErr) {
                    ...
                    connection.query(insertMessageQuery, [id, userEmail, request_email, message], (insertErr, insertResults) => {
                        if (insertErr) {
                            ...
                            // Add a notification for the user using the correct user ID
                            const insertNotificationQuery = 'INSERT INTO notifications (user_id, message) VALUES (?, ?)';
                            connection.query(insertNotificationQuery, [userId, notificationMessage], (notificationErr, notificationResults) => {
                                if (notificationErr) {
...
//   });
  ...
  function fetchAndRenderFinancialReportsDioceseAdmin(res, totalUsers, adminList, userEmail) {
  ...
  // Sample function to fetch and render financial reports for superadmin
  function fetchAndRenderFinancialReportsSystemAdmin(res, totalUsers, adminList, userEmail) {
    const church1 = 'San Roque Parish Church';
    const church2 = 'Sta Rita De Casia';
          ...
          fetchTotalIncome(church2, (errorIncomeStaRita, totalIncomeStaRita) => {
            ...
            fetchNetIncomeLossForSanRoque((errorNetIncomeLossSanRoque, netIncomeLossSanRoque) => {
              ...
              fetchNetIncomeLossForStaRita((errorNetIncomeLossStaRita, netIncomeLossStaRita) => {
                ...
                // Render the 'superadmin' view with net income (loss), total expenses, and total income for each church
                res.render('system_admin/superadmin', {
                  totalUsers,
                  adminList,
                  userEmail,
                  netIncomeLossSanRoque,
